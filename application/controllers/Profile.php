...
class Profile extends AUTH_Controller {
	...
	public function update() {
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if ($result > 0) {
				$this->updateProfil();
				$this->session->set_flashdata('msg', show_succ_msg('Profile Data Successfully changed'));
				redirect('Profile');
			} else {
		...
		if ($this->form_validation->run() == TRUE) {
			if (md5($this->input->post('passLama')) == $this->userdata->password) {
				if ($this->input->post('passBaru') != $this->input->post('passKonf')) {
					$this->session->set_flashdata('msg', show_err_msg('New Password and Password Confirmation must be the same'));
					redirect('Profile');
				} else {
					$data = [
					...
					if ($result > 0) {
						$this->updateProfil();
						$this->session->set_flashdata('msg', show_succ_msg('Password was successfully changed'));
						redirect('Profile');
					} else {
						$this->session->set_flashdata('msg', show_err_msg('Failed password changed'));
						redirect('Profile');
					}
				}
			} else {
