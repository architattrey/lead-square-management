...
class Kota extends AUTH_Controller {
	...
	public function prosesTambah() {
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if ($result > 0) {
				$out['status'] = '';
				$out['msg'] = show_succ_msg('City Data Successfully added', '20px');
			} else {
				$out['status'] = '';
				$out['msg'] = show_err_msg('City Data Failed added', '20px');
			}
		} else {
			$out['status'] = 'form';
	...
	public function prosesUpdate() {
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if ($result > 0) {
				$out['status'] = '';
				$out['msg'] = show_succ_msg('City Data Successfully Updated', '20px');
			} else {
				$out['status'] = '';
				$out['msg'] = show_err_msg('City Data Failed Updated', '20px');
			}
		} else {
			$out['status'] = 'form';
			$out['msg'] = show_err_msg(validation_errors());
	...
	public function delete() {
		...
		if ($result > 0) {
			echo show_succ_msg('Successful City Data is deleted', '20px');
		} else {
			echo show_err_msg('Failed City data is deleted', '20px');
		}
	}
		...
		if ($_FILES['excel']['name'] == '') {
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				if (count($resultData) != 0) {
					...
					if ($result > 0) {
						$this->session->set_flashdata('msg', show_succ_msg('Successful City Data is imported into the database'));
						redirect('Kota');
					}
				} else {
					$this->session->set_flashdata('msg', show_msg('City Data Failed imported into database)', 'warning', 'fa-warning'));
					redirect('Kota');
				}
