...
class Pegawai extends AUTH_Controller {
	...
	public function index() {
		...
		$data['page'] = "Lead";
		$data['judul'] = "Officer Data";
		$data['deskripsi'] = "Manage Lead Data";
	...
	public function prosesTambah() {
		...
		$result = $this->M_pegawai->insert($data);
			...
			if ($result > 0) {
				$out['status'] = '';
				$out['msg'] = show_succ_msg('Lead Data Successfully added', '20px');
			} else {
				$out['status'] = '';
				$out['msg'] = show_err_msg('Lead Data Failed added', '20px');
			}
	...
	public function prosesUpdate() {
		...
		$result = $this->M_pegawai->update($data);
			...
			if ($result > 0) {
				$out['status'] = '';
				$out['msg'] = show_succ_msg('Lead Data Successfully Updated', '20px');
			} else {
				$out['status'] = '';
				$out['msg'] = show_err_msg('Lead Data Failed Updated', '20px');
			}
		/*if ($this->form_validation->run() == TRUE) {
			$result = $this->M_pegawai->update($data);
	...
	public function delete() {
		...
		if ($result > 0) {
			echo show_succ_msg('Lead Data Successfully Deleted', '20px');
		} else {
			echo show_err_msg('Failed Lead Data deleted', '20px');
		}
	}
	...
	public function import() {
		...
		if ($_FILES['excel']['name'] == '') {
			$this->session->set_flashdata('msg', 'The file must be filled in');
		} else {
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				if (count($resultData) != 0) {
					...
					if ($result > 0) {
						$this->session->set_flashdata('msg', show_succ_msg('Lead Data Successfully Imported into Databases'));
						redirect('Pegawai');
					}
				} else {
					$this->session->set_flashdata('msg', show_msg('Lead Data Failed to be imported into the database (Data has been updated)', 'warning', 'fa-warning'));
					redirect('Pegawai');
				}
