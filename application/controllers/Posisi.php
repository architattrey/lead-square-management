...
class Posisi extends AUTH_Controller {
	...
	public function index() {
		...
		$data['page'] 		= "position";
		$data['judul'] 		= "Position Data";
		$data['deskripsi'] 	= "Manage Position Data";
	...
	public function prosesTambah() {
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if ($result > 0) {
				$out['status'] = '';
				$out['msg'] = show_succ_msg('Position Data Successfully added', '20px');
			} else {
				$out['status'] = '';
				$out['msg'] = show_err_msg('Failed Position Data added', '20px');
			}
		} else {
			$out['status'] = 'form';
	...
	public function prosesUpdate() {
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if ($result > 0) {
				$out['status'] = '';
				$out['msg'] = show_succ_msg('Position Data Successfully Updated', '20px');
			} else {
				$out['status'] = '';
				$out['msg'] = show_err_msg('Failed Position Data Updated', '20px');
			}
		} else {
			$out['status'] = 'form';
			$out['msg'] = show_err_msg(validation_errors());
	...
	public function delete() {
		...
		if ($result > 0) {
			echo show_succ_msg('Position Data Successfully Deleted', '20px');
		} else {
			echo show_err_msg('Failed Position Data deleted', '20px');
		}
	}
		...
		if ($_FILES['excel']['name'] == '') {
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				if (count($resultData) != 0) {
					...
					if ($result > 0) {
						$this->session->set_flashdata('msg', show_succ_msg('Successful Position Data is imported into the database'));
						redirect('Posisi');
					}
				} else {
					$this->session->set_flashdata('msg', show_msg('
Position Data Failed to import into database (Data updated)', 'warning', 'fa-warning'));
					redirect('Posisi');
				}
